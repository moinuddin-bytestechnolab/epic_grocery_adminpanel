import { useState, useEffect } from 'react';
import { FiEdit } from 'react-icons/fi';
import { RiDeleteBin6Line } from 'react-icons/ri';
import { TfiSearch } from 'react-icons/tfi';
import { TiArrowSortedDown, TiArrowSortedUp } from 'react-icons/ti';
import Pagination from '../../components/pagination';
import SweetAlert from '../../hooks/SweetAlert';
import AddEditOffer from './AddEditOffer';
import { getOffers } from '../../api';
import { date } from '../../helpers/Date';


const Index = () => {
    // This state is use for handeling open close modal
    const [isModalOpen, setIsModalOpen] = useState(false);

    // This state is use for fetch offers data
    const [getOffersData, setGetOffersData] = useState([])


    // This function is use for open modal
    const openModal = () => {
        setIsModalOpen(true);
    };
    
    // This function is use for close modal
    const closeModal = () => {
        setIsModalOpen(false);
    };
    
    // This function is use for fetching offers data
    const fetchOffers = async () => {
        const res = await getOffers();
        setGetOffersData(res?.data.data)
    }

    useEffect(() => {
        fetchOffers()
    },[])


  return (    
    <div className="">
        {/* This is a modal which is use for update user data */}
        {/* <EditUserModal isOpen={isModalOpen} onClose={closeModal}/> */}

        <div className='flex items-center justify-end bg-white p-3'>
            <h1 className='mr-auto font-bold text-gray-600 text-xl uppercase'>Offers</h1>
            {/* Here is searchbar */}
            <div className='searchbar'>
                <label className="relative block">
                    <span className="sr-only">Search</span>
                    <span className="absolute inset-y-0 left-0 flex items-center pl-2">
                    <TfiSearch className="h-5 w-5 fill-gray-500"/>
                    </span>
                    <input className="placeholder:italic placeholder:text-gray-500 block bg-gray-100 w-80 border border-slate-300 rounded-md py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="Search by user name..." type="text" name="search"/>
                </label>
            </div>

            {/* Add categories AddEdit Modal */}
            <AddEditOffer isOpen={isModalOpen} onClose={closeModal}/>
            <button type="button" onClick={openModal} className="font-medium rounded-lg text-sm px-5 py-2.5 ml-2 text-center inline-flex items-center text-white bg-gray-600 border-2 hover:bg-white hover:text-gray-600 hover:border-2 hover:border-gray-600">
                Add Offers
            </button>

        </div>

        {/* This is a table showing user's */}
        <table className="w-full text-sm text-left text-gray-500">
            <thead className="text-xs text-white uppercase bg-gray-600">
                <tr>
                    <th scope="col" className="px-6 py-3">
                        Category Name 
                    </th>
                    <th scope="col" className="px-6 py-3">
                        Product Name 
                        <span className='float-right cursor-pointer'>
                            <TiArrowSortedUp/>
                            <TiArrowSortedDown/>
                        </span>
                    </th>
                    <th scope="col" className="px-6 py-3">
                        Discount %
                        <span className='float-right cursor-pointer'>
                            <TiArrowSortedUp/>
                            <TiArrowSortedDown/>
                        </span>
                    </th>
                    <th scope="col" className="px-6 py-3">
                        Original Price
                        <span className='float-right cursor-pointer'>
                            <TiArrowSortedUp/>
                            <TiArrowSortedDown/>
                        </span>
                    </th>
                    <th scope="col" className="px-6 py-3">
                        Discount Price
                        <span className='float-right cursor-pointer'>
                            <TiArrowSortedUp/>
                            <TiArrowSortedDown/>
                        </span>
                    </th>
                    <th scope="col" className="px-6 py-3">
                        Start Date
                        <span className='float-right cursor-pointer'>
                            <TiArrowSortedUp/>
                            <TiArrowSortedDown/>
                        </span>
                    </th>
                    <th scope="col" className="px-6 py-3">
                        End Date
                        <span className='float-right cursor-pointer'>
                            <TiArrowSortedUp/>
                            <TiArrowSortedDown/>
                        </span> 
                    </th>
                    <th scope="col" className="px-6 py-3">
                        Status
                        <span className='float-right cursor-pointer'>
                            <TiArrowSortedUp/>
                            <TiArrowSortedDown/>
                        </span> 
                    </th>
                    <th scope="col" className="px-6 py-3">
                        Actions
                    </th>
                </tr>
            </thead>
            <tbody>
                {
                    getOffersData[0] ?
                    getOffersData.map((item : any,index : any) => {
                        return(
                        <tr key={index} className="bg-white border-b">
                            <td className="px-6 py-4">
                                {item.category.name}
                            </td>
                            <td className="px-6 py-4">
                                {item.product.name}
                            </td>
                            <td className="px-6 py-4">
                                {item.discount_percentage}<b>%</b>
                            </td>
                            <td className="px-6 py-4">
                                {item.original_price}
                            </td>
                            <td className="px-6 py-4">
                                { Math.abs(item.original_price * item.discount_percentage / 100 - item.original_price) }
                            </td>
                            <td className="px-6 py-4">
                                { date(item.start_date) }
                            </td>
                            <td className="px-6 py-4">
                                { date(item.end_date) }
                            </td>
                            <td className="px-6 py-4">
                                {
                                    item.is_active === true 
                                    ?
                                    <span className="bg-green-500 rounded p-1 text-white font-medium">Active</span>
                                    :
                                    <span className="bg-red-500 rounded p-1 text-white font-medium">In Active</span>
                                }
                            </td>
                            <td className="px-6 py-4">
                                <div className='flex text-xl'>
                                    <FiEdit onClick={openModal} className="text-blue-500 cursor-pointer mx-1"/>
                                    <RiDeleteBin6Line className="text-red-500 cursor-pointer"/>
                                </div>
                            </td>
                        </tr>
                        )
                    })
                    :
                    <tr className='bg-white border-b text-center font-bold uppercase'>
                        <td colSpan={9} className='px-6 py-4'>
                            <span>Offer's not found</span>
                        </td>
                    </tr>
                }
            </tbody>
        </table>

        {/* This is a pagination */}
        <div className='flex justify-center'>
            {/* <Pagination/> */}
        </div>
    </div>
  )
}

export default Index